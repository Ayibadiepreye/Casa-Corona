...
import {
  LayoutDashboard, UserCircle, Scissors, Package, Image, Star,
  MessageSquare, BarChart3, Receipt, Settings, LogOut, Menu, X, ChevronRight, Zap, Store,
  CalendarDays, Bell,
} from "lucide-react";
...
const navItems = [
  ...
  { href: "/vendor/services", label: "Services", icon: Scissors },
  ...
  { href: "/vendor/payments", label: "Payments", icon: Receipt },
...
export function VendorLayout({ children }: { children: React.ReactNode }) {
  const { user, signOut, token, loading } = useAuth();
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <aside className={`flex flex-col h-full bg-card border-r shrink-0 transition-all duration-300 ${!mobile && collapsed ? "w-16" : "w-64"}`}>
      ...
      <div className={`p-3 border-t space-y-1 ${collapsed && !mobile ? "flex flex-col items-center" : ""}`}>
        <Link
          href="/browse"
          title={collapsed && !mobile ? "Browse Marketplace" : undefined}
          className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-foreground/70 hover:bg-muted hover:text-foreground transition-all ${collapsed && !mobile ? "justify-center px-2" : ""}`}
        >
          <Store size={17} className="shrink-0" />
          {(!collapsed || mobile) && "Browse Marketplace"}
        </Link>
        {(!collapsed || mobile) && (
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="md:hidden flex items-center justify-between px-4 h-16 border-b bg-background/90 backdrop-blur shrink-0">
          ...
          <span className="font-serif font-bold text-lg">Vendor Panel</span>
